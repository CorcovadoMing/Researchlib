...
class _CoordinatesEncoding2d(nn.Module):
    ...
    def forward(self, x):
        ...
        y_coords = torch.arange(image_height).unsqueeze(1).expand(image_height, image_width).float()
        x_coords = torch.arange(image_width).unsqueeze(0).expand(image_height, image_width).float()
