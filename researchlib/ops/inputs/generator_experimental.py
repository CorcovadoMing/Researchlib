...
class _GeneratorDev(nn.Module):
    ...
    def __init__(self, 
        ...
        for i in preprocessing_list:
            try:
                if 'albumentations' in str(type(i)):
...
#     x = x.astype(np.uint8)
#     y = y.astype(np.int64) if not do_y else y.astype(np.uint8)
...
#             if self.worker > 0:
#                 ds = MapData(ds, self.train_processing_function)
#                 ds = MultiProcessRunnerZMQ(ds, self.worker, self.buffer*self.batch_size)
#             else:
#                 ds = MapData(ds, self.train_processing_function)
#             ds = BatchData(ds, self.batch_size, remainder = True)
#             if self.data_info:
