...
def AutoConvNet2d(input_dim, blocks, pooling_factor=2, pooling_freq=1, activator=nn.ELU, flatten=True):
    layers = []
    in_dim = input_dim
    out_dim = 64
...
def AutoConvTransposeNet2d(input_dim, blocks, pooling_factor=2, pooling_freq=1, activator=nn.ELU, flatten=False):
    layers = []
    in_dim = input_dim
    out_dim = 64
    for i in range(blocks):
