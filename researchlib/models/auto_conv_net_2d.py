...
def AutoConvNet2d(input_dim, blocks, type='vgg', start_filter=128, max_filter=1024, pooling_factor=2, pooling_freq=1, norm='batch', activator=nn.ELU, flatten=False, preact=True):
    if type =='residual': _op_type = block.ResBlock2d
    elif type =='se': _op_type = block.ResBlock2d
    elif type =='vgg': _op_type = block.ConvBlock2d
    
    se = True if type=='se' else False
...
def AutoConvTransposeNet2d(input_dim, blocks, start_filter=1024, min_filter=128, pooling_factor=2, pooling_freq=1, norm='batch', activator=nn.ELU, flatten=False, preact=True):
    if type =='residual': _op_type = block.ResBlock2d
    elif type =='se': _op_type = block.ResBlock2d
    elif type =='vgg': _op_type = block.ConvBlock2d
