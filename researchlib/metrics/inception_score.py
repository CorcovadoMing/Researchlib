...
class FID(Matrix):
    ...
    def output(self):
        real_mu, real_sigma = torch.mean(self.real_pool, 0), torch_cov(self.real_pool, rowvar=False)
        fake_mu, fake_sigma = torch.mean(self.fake_pool, 0), torch_cov(self.fake_pool, rowvar=False)
        #fid = torch_calculate_frechet_distance(fake_mu, fake_sigma, real_mu, real_sigma).numpy()
        fid = calc_fid(fake_mu.numpy(), fake_sigma.numpy(), real_mu.numpy(), real_sigma.numpy())
        return {'fid': fid}
