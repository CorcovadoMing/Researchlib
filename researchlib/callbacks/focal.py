...
class AdaptiveFocalUpdate(Callback):
    ...
    def on_iteration_end(self, **kwargs):
        if kwargs['model'].training == False:
            ...
            if self.total_map == None: self.total_map = [0 for _ in range(output.size(1))]
            if self.correct_map == None: self.correct_map = [0 for _ in range(output.size(1))]
    ...
    def on_validation_end(self, **kwargs):
        alpha = [(float(self.correct_map[i])/self.total_map[i]) for i in range(len(self.total_map))]
        print(alpha)
