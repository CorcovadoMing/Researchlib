...
class EBConv2d(Function):
    ...
    def forward(ctx, inp, weight, bias, stride, padding, dilation, groups):
        ...
        f(
            backend.library_state, inp, output, weight, bias, columns, ones, kH, kW, ctx.stride[0],
    ...
    def backward(ctx, grad_output):
        ...
        f = getattr(backend, 'SpatialConvolutionMM_updateOutput')
        f(
            backend.library_state, inp, new_output, wplus, None, columns, ones, kH, kW,
        ...
        g(
            backend.library_state, inp, normalized_grad_output, grad_inp, wplus, columns, ones, kH,
...
class EBAvgPool2d(Function):
    ...
    def forward(
        ...
        backend.SpatialAveragePooling_updateOutput(
            backend.library_state, inp, output, ctx.kernel_size[1], ctx.kernel_size[0],
            ctx.stride[1], ctx.stride[0], ctx.padding[1], ctx.padding[0], ctx.ceil_mode,
    ...
    def backward(ctx, grad_output):
        ...
        backend.SpatialAveragePooling_updateGradInput(
            backend.library_state, inp, normalized_grad_output, grad_inp, ctx.kernel_size[1],
            ctx.kernel_size[0], ctx.stride[1], ctx.stride[0], ctx.padding[1], ctx.padding[0],
...
class _Interpreter:
    ...
    def excitation_backprop(self, model, img, label, contrastive = False, plot = True):
        ...
        def new_conv2d(self, x):
            return EBConv2d.apply(
                x, self.weight, self.bias, self.stride, self.padding, self.dilation, self.groups
            )

        def new_avgpool2d(self, x):
            return EBAvgPool2d.apply(
        ...
        if contrastive:
            ...
            attmap = grad(
                hook_forward_buffer[-1], hook_forward_buffer[0], diff, create_graph = True
            )[0]
        else:
