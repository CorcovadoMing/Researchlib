...
class InceptionResidualV2A(_Block):
    def __postinit__(self):
        ...
        self.branch2 = nn.Sequential(
            unit_fn(self.op, self.in_dim, remain_hidden, False, True, False,
                                 **self._get_custom_kwargs({'kernel_size': 1, 'stride': 1, 'padding': 0, 'erased_activator': False})),
            unit_fn(self.op, remain_hidden, remain_hidden, False, True, False,
                                 **self._get_custom_kwargs({'kernel_size': 3, 'stride': 1, 'padding': 1, 'erased_activator': False})),
            unit_fn(self.op, remain_hidden, remain_hidden, False, True, False,
                                 **self._get_custom_kwargs({'kernel_size': 3, 'stride': 1, 'padding': 1, 'erased_activator': False}))
        )

        self.conv2d = unit_fn(self.op, self.out_dim, self.out_dim, False, True, False,
                                 **self._get_custom_kwargs({'kernel_size': 1, 'stride': 1, 'padding': 0, 'erased_activator': False}))
...
class InceptionResidualV2B(_Block):
    def __postinit__(self):
        unit_fn = self._get_param('unit', unit.conv)
...
class InceptionResidualV2C(_Block):
    def __postinit__(self):
        unit_fn = self._get_param('unit', unit.conv)
        hidden_dim = self.out_dim // 2
        remain_hidden = self.out_dim - (1 * hidden_dim)
