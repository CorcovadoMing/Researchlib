...
class _InvertedBottleneckBlock(_Block):
    ...
    def __postinit__(self):
        ...
        pre_conv_layers = [
            unit_fn(
                self.op, self.in_dim, hidden_size, False, self.do_norm, False,
                **first_custom_kwargs
            ),
            unit_fn(
                self.op, hidden_size, hidden_size, False, self.do_norm, False,
                **second_custom_kwargs
            ),
        ]
        self.pre_conv = nn.Sequential(*list(filter(None, pre_conv_layers)))
        self.post_conv = unit_fn(
            self.op, hidden_size, self.out_dim, False, self.do_norm, False, **third_custom_kwargs
        )
