...
def _branch_function(config, parameter_manager, **kwargs):
    ...
    conv_op = [
        config._unit(f'{config.prefix}_m1', op.__dict__[f'SKConv{config.dim}'], config.in_dim, config.out_dim, **first_conv_kwargs),
        op.Downsample(channels = config.out_dim, filt_size = 3, stride = config.stride) if config.blur else None,
        config._unit(f'{config.prefix}_m2', op.__dict__[f'SKConv{config.dim}'], config.out_dim, config.out_dim, **second_conv_kwargs), 
...
def _SKBlock(prefix, _unit, _op, in_dim, out_dim, **kwargs):
    '''
