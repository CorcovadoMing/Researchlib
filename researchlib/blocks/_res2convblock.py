    ...
    def forward(self, x):
        ...
        for s in range(rang):
            if s == 0:
                ...
                ys.append(self.act_layer(self.norm_group[s-1](self.op_group[s-1](xs[s]))))
            else:
                ys.append(self.act_layer(self.norm_group[s-1](self.op_group[s-1](xs[s] + ys[-1]))))
        if self.ignore_last:
            ys.append(xs[-1])
        out = torch.cat(ys, 1)
        return out
...
class Res2ConvBlock(_Block):
    ...
    def __postinit__(self):
        ...
        if conv_kwargs['kernel_size'] == 1:
            ...
            group, dim, ignore_last = len(test_case), len(test_case[0]), len(test_case[-1]) != len(test_case[0])
            dim = math.ceil(self.in_dim / scales)
            norm_group = [self._get_norm_layer(norm_type, dim) for _ in range(group-1)]
            stride = conv_kwargs['stride']
            conv_kwargs['stride'] = 1
            op_group = [self.op(dim, dim, **conv_kwargs) for _ in range(group-1)]
            conv_layer = []
            if stride != conv_kwargs['stride']:
                conv_layer.append(self.op(self.in_dim, self.in_dim, 1, stride=stride))
            conv_layer.append(_inner_res2conv(op_group, norm_group, self._get_activator_layer(activator_type), scales, ignore_last)) 
