...
class _conv(_Block):
    def __postinit__(self):
        ...
        pool_type = self._get_param('pool_type', 'Upsample') if self._is_transpose(
        ) else self._get_param('pool_type', 'MaxPool')
        ...
        pool_layer = self._get_pool_layer(
            pool_type, pool_factor, self.out_dim
        ) if self.do_pool else None
