...
class _inner_res2conv(nn.Module):
    ...
    def forward(self, x):
        ...
        ys = []
        rang = self.group-1 if self.ignore_last else self.group
        for s in range(rang):
            if s == 0:
                ys.append(xs[s])
            elif s == 1:
                ys.append(self.op_group[s-1](xs[s]))
            else:
                ys.append(self.op_group[s-1](xs[s] + ys[-1]))
        if self.ignore_last:
            ys.append(xs[-1])
        out = torch.cat(ys, 1)
        return out
...
class _res2(_Block):
    ...
    def __postinit__(self):
        ...
        if conv_kwargs['kernel_size'] == 1:
            ...
            op_group = [_conv(self.op, dim, dim, False, True, self.preact) for _ in range(group-1)]
            conv_layer = []
