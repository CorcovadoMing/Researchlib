...
def COIL100(name, is_train, shuffle = True, merge_train_val = True, resize = None):
    if is_train:
        if merge_train_val:
            x = np.concatenate([np.load(os.path.join(this_dir, f'{name}_x.npy')), np.load(os.path.join(this_dir, f'{name}_testx.npy'))], 0)
            y = np.concatenate([np.load(os.path.join(this_dir, f'{name}_y.npy')), np.load(os.path.join(this_dir, f'{name}_testy.npy'))], 0)
        else:
