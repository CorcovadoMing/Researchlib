...
class benchmark(object):
    ...
    def genesis(self, categories):
        ...
        ans = input('Are you sure reset these benchmarks: {}? [y/n]'.format(
            categories))
        if ans in ['y', 'Y']:
            for cat in categories:
                # delete if not exist
                ...
                # insert the dummy rows, cols will be updated by incomming edition
                wks.insert_rows(
                    wks.frozen_rows,
                    number=1,
                    values=[
                        'Hello World, Cols will be updated by incomming submissions.'
                    ],
                    inherit=False)
                wks.frozen_rows = 1
                wks.frozen_cols = 1
    ...
    def update_from_runner(self, category, time_id, description):
        ...
        # filter out new cols
        diff_cols = list(set(new_cols) - set(self.worksheet_cols))
        if len(diff_cols) > 0:
            # update worksheet_cols
            self.worksheet_cols = self.worksheet_cols + diff_cols
            updated_cols = self._primary_keys + self.worksheet_cols
            wks.update_row(index=1, values=new_cols, col_offset=0)
        ...
        for col in self.worksheet_cols:
        ...
        time_id_list = wks.get_col(col=1,
                                   returnas='matrix',
        ...
        if time_id_idx >= 0:
            ...
            wks.insert_rows(wks.frozen_rows,
                            number=1,
