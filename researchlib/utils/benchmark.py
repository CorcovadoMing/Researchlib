...
client_secret_path = os.path.join(module_path, '../.credential/client_secret_855251407115-slk3rv4qfi6b6narp2hapov9lv6iipjt.apps.googleusercontent.com.json')
...
class benchmark(object):
    ...
    def __init__(self, client_secret_path=client_secret_path):
        ...
        self.categories = ['Classification', 'Segmentation', 'GAN', 'AnomalyDetection']
        ...
        self._primary_keys = ['time_id',]
            
            
    def verify_name(self, name):
        if name not in self.categories:
            raise ValueError('{} is not in the {}'.format(name, self.categories))
    ...
    def genesis(self, categories):
        ...
        ans = input('Are you sure reset these benchmarks: {}? [y/n]'.format(categories))
        if ans in ['y', 'Y']:
            for cat in categories:
                # delete if not exist
                try:
                    ...
                    print('Failed to delete {}, because it is not exist.'.format(cat))
                # create new
...
#                 wks = sh.add_worksheet(self._worksheet_name, rows=1, cols=2)
    ...
    def update_from_runner(self, category, time_id, description):
        ...
        insert_values = [time_id,] # for self._primary_keys
