...
class _SASA2d(nn.Module):
    ...
    '''
    def __init__(self, in_dim, out_dim, kernel_size, stride=1, padding=0, groups=1, bias=False, **kwargs):
        super().__init__()
        ...
        if kernel_size == 1:
            ...
            self.relative_column_encoding = torch.nn.Embedding(self.kernel_size[1], int(out_dim/2))
    ...
    def forward(self, x):
        ...
        _, _, w, h = x.size()
        x = F.unfold(x, self.kernel_size, 1, self.padding, self.stride).reshape(x.size(0), self.in_dim, self.kernel_flatshape, -1)
        q = self.wq(x)[:, :, self.q_pos, :].unsqueeze(2)
        k = self.wk(x)
        v = self.wv(x)
        
        embedded = torch.cat([self.relative_row_encoding(self.row.to(x.device)), self.relative_column_encoding(self.column.to(x.device))], dim=1).transpose(-1, -2)
        embedded = embedded[None, :, :, None].expand_as(k).to(x.device)
        
        att = F.softmax(torch.einsum('bmik,bmjk -> bmij', q, k) + torch.einsum('bmik,bmjk -> bmij', q, embedded), -1)
        return torch.einsum('bmij,bmjk -> bmjk', att, v).sum(2).reshape(x.size(0), self.out_dim, int(w/self.stride[0]), int(h/self.stride[1]))