...
class _MHA2d(nn.Module):
    ...
    def forward(self, x):
        ...
        wq = self.wq(x).view(b, h * w, -1, self.heads).permute(
            (0, 3, 1, 2)).reshape(b * self.heads, h * w, -1)  # B*Heads,H*W,dkh
        wk = self.wk(x).view(b, h * w, -1, self.heads).permute(
            (0, 3, 1, 2)).reshape(b * self.heads, h * w, -1)
        wv = self.wv(x).view(b, h * w, -1, self.heads).permute(
            (0, 3, 1, 2)).reshape(b * self.heads, h * w, -1)
        o = F.softmax((torch.bmm(wq, wk.transpose(-1, -2))) /
                      ((self.dk // self.heads)**0.5), -1)
        o = torch.bmm(o, wv)
        o = o.view(b, self.heads, h * w, -1).permute(
            (0, 2, 3, 1)).reshape(b, h * w, self.dv)
        o = self.wo(o)
...
class _AAConv2d(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv = nn.Conv2d(
            in_channels, out_channels, 3, stride, 1, bias=bias)
