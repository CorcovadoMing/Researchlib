...
class _blur_function_backward(Function):
    ...
    def forward(ctx, grad_output, kernel, kernel_flip):
        ...
        grad_input = F.conv2d(
            grad_output, kernel_flip, padding=1, groups=grad_output.size(1))
        return grad_input
    ...
    def backward(ctx, gradgrad_output):
        ...
        grad_input = F.conv2d(
            gradgrad_output, kernel, padding=1, groups=gradgrad_output.size(1))
...
class _blur_function(Function):
    ...
    def backward(ctx, grad_output):
        kernel, kernel_flip = ctx.saved_tensors
        grad_input = _blur_function_backward.apply(grad_output, kernel,
                                                   kernel_flip)
        return grad_input, None, None
