...
class _Mask(nn.Module):
    def __init__(self, down_unit, up_unit, in_dim, out_dim, kenel_size=3, norm='batch', activator=nn.ELU, pooling_factor=2, preact=True, se=False):
        super().__init__()
        self.down_unit = down_unit(in_dim, out_dim, kenel_size, norm, activator, True, 'maxpool', pooling_factor, preact, se)
        self.up_unit = up_unit(out_dim, out_dim, kenel_size, norm, activator, True, 'interpolate', pooling_factor, preact, se)
...
class _AttentionBlock2d(nn.Module):
    def __init__(self, down_unit, up_unit, in_dim, out_dim, norm='batch', activator=nn.ELU, pooling=True, pooling_type='combined', pooling_factor=2, preact=True, se=False):
        ...
        self.trunk_branch = builder([
            down_unit(in_dim, out_dim, 3, norm, activator, False, pooling_type, pooling_factor, preact, se),
            down_unit(out_dim, out_dim, 3, norm, activator, False, pooling_type, pooling_factor, preact, se)
        ])
        self.mask_branch = _Mask(down_unit, up_unit, in_dim, out_dim, 3, norm, activator, pooling_factor, preact, se)
